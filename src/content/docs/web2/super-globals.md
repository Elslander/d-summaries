...
- _Relative URL_: current or different page on the same website.
- _Absolute URL_: external website.
...
- You can pass in variables with the "querystring", separated by ampersands. _Querystring_: the part after the question mark.
  It is grouped by _key=value&key=value_.
...
- _Querystrings_ and _post requests_ are potential security weaknesses: always validate these inputs.
...
- The include and require statements are identical except upon failure:
  - _require_ will produce a fatal error (E_COMPILE_ERROR) and stop the script;
  - _include_ will only produce a warning (E_WARNING) and the script will continue.
